...
function PATCHER_applyOnExec(targetExec){
	...
	if (USE_MIX !== false || USE_IEDIT !== false){
		if (PATCHER_patchDecompiled !== undefined && fs.existsSync(targetExec) !== false){
			...
			PATCHER_arquivoBruto = fs.readFileSync(targetExec, 'hex');
			//
			var RE3_FILE_END;
			var RE3_FILE_START;
			var PATCHER_FINAL_FILE = '';
			if (USE_MIX === true){
				RE3_FILE_START = PATCHER_arquivoBruto.slice(0, MIX_fileTypes[PATCHER_platform][1]);
				RE3_FILE_END = PATCHER_arquivoBruto.slice(MIX_fileTypes[PATCHER_platform][2], PATCHER_arquivoBruto.length);
				PATCHER_FINAL_FILE = RE3_FILE_START + MIX_Database + RE3_FILE_END;
			}
			if (USE_IEDIT === true){
				RE3_FILE_START = PATCHER_arquivoBruto.slice(0, IEDIT_fileTypes[PATCHER_platform][1]);
				RE3_FILE_END = PATCHER_arquivoBruto.slice(IEDIT_fileTypes[PATCHER_platform][2], PATCHER_arquivoBruto.length);
