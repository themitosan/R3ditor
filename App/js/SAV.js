...
function finalizeSave(){
	var FILE = SAVE_INDICADOR_HEADER + 	localStorage.Save_1 + localStorage.Save_2 + localStorage.Save_3 + localStorage.Save_4 +
	localStorage.Save_5 + localStorage.Save_6 + localStorage.Save_7 + localStorage.Save_8 + localStorage.Save_9 + localStorage.Save_10 + 
...
function save_renderInfos(s_slot){
	try{
		...
		document.getElementById('lbl-maps').innerHTML = '[WIP] -  (HEX: ' + mapExtractA.toUpperCase() + mapExtractB.toUpperCase() + ')';
		// Room / Event [WIP]
...
function save_renderLife(s_slot){
	...
	if (POISON[veneno][0] === 'Yes'){
		...
		// Status: DEAD! (Ou Hex FFFF FFFF FFFF FFFF)
		if (HP < 0){
			STATUS = 'Dead';
			co = 'txt-fine';
		}
...
function SAV_ADD_ITEM_BOX(PERSON, INDEX, ITEMHEX, QUANTIDADE, ATRIBUTO, VNULO){
	...
	var ITEM_BOX_HTML_TEMPLATE = '<div class="bau-item" id="item-' + INDEX + '"><img src="img/items/' + ITEMHEX + '.png" draggable="false" class="b-icon" id="boxicon-' + INDEX + '" ' + 
	'onclick="SAV_addInfo(' + PERSON + ', \'' + ITEMHEX + '\');"><font class="b-quant-lbl" id="b-' + PERSON + '-q-lbl-' + INDEX + '" onclick="SAV_addInfo(' + PERSON + ', \'' + ITEMHEX + 
	'\');" style="color: ' + colo + ';text-shadow: ' + cfundo + ';">' + quan + '</font><div class="b-label"><font class="' + cssfix + '" id="b-name-' + INDEX + '" onclick="SAV_addInfo(' + 
	PERSON + ', \'' + ITEMHEX + '\');">(' + INDEX + ') ' + nome + '</font></div><input type="button" class="btn-box ' + cssfixbtn + '" onclick="showModItem(2, ' + PERSON + ', ' + INDEX + 
	', \'' + ITEMHEX + '\');" value="Modify"></div>';
	if (PERSON === 0){
		$('#JILL-BOX').append(ITEM_BOX_HTML_TEMPLATE);
	} else {
		$('#CARLOS-BOX').append(ITEM_BOX_HTML_TEMPLATE);
	}
	// Percentage Fix
